...
export class CreateFlagComponent implements OnInit, AfterViewInit {
  ...
  updateFlag() {
    let flag1: FlagModel | any
    let flag2: FlagModel | any
    ...
    index = this.timeLine.time_line.flags?.findIndex((timestamp: FlagModel) => timestamp.date_obj.timestamp === this.editFlag.date_obj.timestamp);
    ...
    if (find.length >= 1) {
      ...
      if (this.editFlag.edit === 'edit-flag-1') {
        this.timeLine.time_line.flags[index] = this.flagsForm.controls[0].value
        // find = 
        if (flag2) {
          this.timeLine.time_line.flags[index].flags2 = [flag2]
        }
      } else if (find.length >= 1 && this.editFlag.edit === 'edit-flag-2') {
